...
The next argument to note is `conditionOnRoot="true"`. **Conditioning** is a way of altering the model and/or the parameter space explored to fit our prior knowledge about how our evolutionary process has taken place. The different directions of time in the two models means that this is fundamentally different between coalescent and birth-death models. Our coalescent model is conditioned on fitting the input data, our sampled phylogeny. In birth-death models, our sampled phylogeny is instead an output of the process, and so we need to provide additional information about how the model should work. In BDSKY, there are two main ways to condition our model: determining when the evolutionary process is considered to have started, and whether (or how) the process has been sampled.
